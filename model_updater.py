            ...
            for folder in ['bpmn_models', 'embedded_matix', 'event_logs', 
                ...
                if folder == 'bpmn_models':
                    ...
                    shutil.copy(os.path.join(src_dir, 
                                             self.complete_name+'.bpmn'), 
                                os.path.join(dest_dir, 
                                             self.modified_name+'_upd.bpmn'))
                    shutil.copy(os.path.join(src_dir, 
                                             self.complete_name+'_meta.json'), 
                                os.path.join(dest_dir, 
                                             self.modified_name+'_upd_meta.json'))
                elif folder == 'embedded_matix':
                    src_dir = self.parms['gl']['embedded_path']
                    shutil.copy(os.path.join(src_dir, 
                                             'ac_'+self.modified_name+'_upd.emb'), 
                                os.path.join(dest_dir, 
                                             'ac_'+self.modified_name+'_upd.emb'))
                    shutil.copy(os.path.join(src_dir, 
                                             self.modified_name+'_upd_emb.h5'), 
                                os.path.join(dest_dir, 
                                             self.modified_name+'_upd_emb.h5'))
