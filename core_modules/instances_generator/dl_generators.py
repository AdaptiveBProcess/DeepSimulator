...
class DeepLearningGenerator:
    ...
    def __init__(self, ia_train, ia_validation, params):
        """constructor"""
        self.times = None
        ...
        self.ia_validation = ia_validation
        self.parms = params
    ...
    def _load_model(self) -> None:
        model_path = os.path.join(self.parms['ia_gen_path'], f"{self.parms['file'].split('.')[0]}_dl{Fe.H5}")
        ...
        if not model_exist or self.parms['update_ia_gen']:
            ...
            save, metadata_file = self._compare_models(acc, self.parms['update_ia_gen'], model_path)
            if save:
                self._save_model(metadata_file, acc)
            else:
                shutil.rmtree(self.temp_output)
            # Save basic features scaler
            name = self.model_path.replace(f'_dl{Fe.H5}', '')
            dump(scaler, open(name + '_ia_scaler.pkl', 'wb'))

    def _compare_models(self, acc, model_exist, file):
        metadata_file = os.path.join(self.parms['ia_gen_path'], f"{self.parms['file'].split('.')[0]}_dl_meta{Fe.JSON}")
    ...
    def _discover_model(self, scaler):
        n_size = 5
        ia_valdn = copy.deepcopy(self.ia_validation)
        # Transform features
        ...
        # model training
        model = self._create_model(serie_trn.shape[1], serie_trn.shape[2], 1)
        model = self._train_model(model, (serie_trn, y_serie_trn), (serie_val, y_serie_val))
    ...
    def _save_model(self, metadata_file, acc):
        ...
        destination = os.path.join(self.parms['ia_gen_path'], f"{self.parms['file'].split('.')[0]}_dl{Fe.H5}")
        source = os.path.join(self.temp_output, self.parms['file'].split('.')[0] + f'_dl{Fe.H5}')
    ...
    def _vectorize(self, log, cols, ngram_size):
        ...
        ohe_df = ku.to_categorical(dt_prefixes[week_col].to_numpy().reshape(-1, 1), num_classes=7)
        # Create a Pandas DataFrame of the hot encoded column
        ohe_df = pd.DataFrame(ohe_df)
        ohe_df.rename(columns={col: 'day_' + str(col) for col in ohe_df.columns}, inplace=True)
        # Concat with original data
        dt_prefixes = pd.concat([dt_prefixes, ohe_df], axis=1).drop([week_col], axis=1)
        dt_prefixes.drop(columns={'ngram_num'}, inplace=True)
        num_columns = len(dt_prefixes.columns)
        dt_prefixes = dt_prefixes.to_numpy().reshape(num_samples, ngram_size, num_columns)
    ...
    def _transform_features(split, scaler):
        split[['inter_time']] = scaler.transform(split[['inter_time']])
    ...
    def _create_model(self, n_size, in_size, out_size):
        ...
        gru_1 = GRU(100, return_sequences=True, dropout=0.2, implementation=1)(series_input)
        gru_2 = GRU(100, return_sequences=False, dropout=0.2, implementation=1)(gru_1)
    ...
    def _train_model(self, model, train_data, valdn_data):
        ...
        # Output file
        output_path = os.path.join(self.temp_output, f"{self.parms['file'].split('.')[0]}_dl{Fe.H5}")
        model_checkpoint = ModelCheckpoint(output_path, monitor='val_loss', verbose=0, save_best_only=True,
                                           save_weights_only=False, mode='auto')
        lr_reducer = ReduceLROnPlateau(monitor='val_loss', factor=0.5, patience=10, verbose=0, mode='auto',
                                       min_delta=0.0001, cooldown=0, min_lr=0)
        model.fit(train_data[0], train_data[1], validation_data=valdn_data, verbose=2,
                  callbacks=[early_stopping, lr_reducer, model_checkpoint], epochs=self.parms['epochs'])
        return model
    ...
    def generate(self, num_instances, start_time, n_size=5):
        ...
        for i in range(0, num_instances):
            ...
            times.append({'caseid': 'Case' + str(i + 1), 'timestamp': timestamp})
            time = timestamp.time()
