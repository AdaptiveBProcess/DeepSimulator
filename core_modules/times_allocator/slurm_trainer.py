...
class SlurmWorker:
    ...
    def __init__(self, argv):
        ...
        self.parms = dict()
        try:
            opts, _ = getopt.getopt(argv, "h:p:f:r:e:", ['parms_file=', 'output_folder=', 'res_files=', 'emb_file='])
            for opt, arg in opts:
            ...
            sys.exit(2)
        # Output Files
        self.temp_output = os.path.join(os.getcwd(), self.parms['output_folder'])
        ...
        print(self.parms)
        self.log_train = pd.read_csv(os.path.join(self.temp_output, 'opt_parms', 'log_train.csv'))
        self.log_valdn = pd.read_csv(os.path.join(self.temp_output, 'opt_parms', 'log_valdn.csv'))
    ...
    def catch_parameter(opt):
        ...
        switch = {'-h': 'help', '-p': 'parms_file', '-f': 'output_folder', '-r': 'res_files', '-e': 'emb_file'}
        return switch[opt]

    def load_parameters(self):
        # Loading of parameters from training
        path = os.path.join(self.temp_output, 'opt_parms', self.parms['parms_file'])
    ...
    def exec_pipeline(self):
        ...
        train_vec = vectorizer.vectorize(self.parms['model_type'], self.log_train, self.parms)
        valdn_vec = vectorizer.vectorize(self.parms['model_type'], self.log_valdn, self.parms)
        # Train
        trainer = self._get_trainer(self.parms['model_type'])
        tf.compat.v1.reset_default_graph()
        model = trainer(self.ac_weights, train_vec, valdn_vec, self.parms)
        # evaluation
